...
# 2. 첫 실행 (리터럴 경로)
claude -p --output-format json --effort high '--disallowedTools=Skill(codex)' "Implement scratch" >/tmp/a1b2c3d4.json 2>/tmp/a1b2c3d4.log

# 3. 세션 ID, 응답, 모델별 USD 비용 추출 (json)
jq -r '(if type == "array" then .[] else . end) | select((.type? // "result") == "result") | .session_id, .result, ({modelUsd: ((.modelUsage // {}) | with_entries(.value = (.value.costUSD // 0)))} | @json)' /tmp/a1b2c3d4.json
...
Append suggestions.
EOF
jq -r '(if type == "array" then .[] else . end) | select((.type? // "result") == "result") | .session_id, .result, ({modelUsd: ((.modelUsage // {}) | with_entries(.value = (.value.costUSD // 0)))} | @json)' /tmp/a1b2c3d4b.jsonl
```
...
`--output-format json` 출력은 버전에 따라 단일 result 객체 또는 이벤트 배열.
`stream-json`은 `--verbose`와 함께 사용하며 JSON Lines로 저장. jq 필터는 두 형태
모두에서 result 객체/이벤트 추출.
...
- 세션 재개 시 출력 파일은 덮어쓰지 않도록 suffix 변경 (`a1b2c3d4b.json` 등)
- `json`, `stream-json`별 jq 필터로 세션 ID와 모델별 비용 확인
...
- `--effort <level>`: `max`(계획), `high`(구현), `medium`(기본)
