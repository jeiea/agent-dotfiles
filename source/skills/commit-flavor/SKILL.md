...
- conventional commit 같다면 아래 중 첫번째로 일치하는 접두사 사용
  - test: 테스트 관련만 수정
  - style: 저장소 린트, 스타일 규칙만 수정
  - docs: 문서만 수정
  - chore, ci: 산출물 코드 변경이 없음
  - refactor: 테스트 코드 변경이 없음
  - feat: 산출물 사용자가 알아야 할 내용이 있음
  - fix: 테스트와 프로덕션 코드를 둘 다 수정함 또는 그 외
- 제목과 본문은 '무엇'보다 변경 이유 우선
  - 선호: 레이스 컨디션 방지
  - 지양: exists 제거
- 본문에 한해서
  - 제목 언어 사용
...
- 이슈, PR 코멘트, 슬랙 스레드 등 배경·근거 URL 보존
  - 서드파티 추적 인자만 제거, 해시까지 보존
...
- 순서대로 읽을 때 '왜?'라고 의문이 든다면 그 이유로 대체
- 커밋 단위는 작업 순서, 접두사가 아니라 의도 단위
  - 나머지 커밋 없이 체리픽·철회를 가정할 때 근거, 문서, 테스트 불일치가 생기면
    푸시하지 않은 커밋을 amend
